...
WAIT_MSG = """"<b>Sedang di Proses ...</b>"""

REPLY_ERROR = """<code>Gunakan Perintah ini dengan Cara Reply Pesan yang Sudah ada Di Bot.</code>"""
    ...
    if len(text)>7:
        ...
        if len(argument) == 3:
            ...
            try:
                ...
                return
        temp_msg = await message.reply("Mohon Menunggu Ya Sayang...")
        try:
...
async def send_text(client: Bot, message: Message):
    if message.reply_to_message:
        ...
        pls_wait = await message.reply("<i>Broadcasting Message.. Ini Akan Memerlukan Sedikit Waktu</i>")
        for row in query:
            chat_id = int(row[0])
            try:
...
Akun Yang Di Hapus: <code>{deleted}</code>
